...
const shots: Shot[] = [
  { name: "Cover Drive", path: [{ x: 0, y: 0 }, { x: 22, y: -16 }, { x: 76, y: -54 }], risk: 41, zone: "Extra Cover" },
  { name: "Straight Drive", path: [{ x: 0, y: 0 }, { x: 0, y: -26 }, { x: 0, y: -92 }], risk: 35, zone: "Long Off / Long On" },
  { name: "Pull Shot", path: [{ x: 0, y: 0 }, { x: -28, y: -10 }, { x: -84, y: -32 }], risk: 63, zone: "Deep Midwicket" },
  { name: "Hook Shot", path: [{ x: 0, y: 0 }, { x: -32, y: 12 }, { x: -86, y: 40 }], risk: 72, zone: "Fine Leg" },
  { name: "Sweep", path: [{ x: 0, y: 0 }, { x: -26, y: 20 }, { x: -76, y: 55 }], risk: 58, zone: "Deep Square Leg" },
  { name: "Reverse Sweep", path: [{ x: 0, y: 0 }, { x: 26, y: 22 }, { x: 80, y: 52 }], risk: 69, zone: "Third Man" },
  { name: "Cut Shot", path: [{ x: 0, y: 0 }, { x: 36, y: 10 }, { x: 92, y: 26 }], risk: 46, zone: "Deep Point" },
  { name: "Lofted Drive", path: [{ x: 0, y: 0 }, { x: 20, y: -30 }, { x: 48, y: -88 }], risk: 78, zone: "Long Off" },
  { name: "Paddle Scoop", path: [{ x: 0, y: 0 }, { x: -10, y: 30 }, { x: -28, y: 92 }], risk: 67, zone: "Fine Leg" },
  { name: "Upper Cut", path: [{ x: 0, y: 0 }, { x: 34, y: 30 }, { x: 86, y: 76 }], risk: 74, zone: "Third Man" },
];
...
const detectPosition = (point: Point, battingHand: MatchContext["battingHand"]) => {
  ...
  if (x > 0 && angle > 8 && angle <= 32) return radius > 62 ? "Deep Point" : angle > 20 ? "Backward Point" : "Point";
  if (x > 0 && angle > -30 && angle <= 8) return radius > 62 ? "Deep Extra Cover" : "Cover";
  if (x > 0 && angle > -62 && angle <= -30) return radius > 62 ? "Deep Extra Cover" : "Extra Cover";
  if (x > -8 && x < 18 && y < -12) return radius > 62 ? "Long Off" : "Mid Off";
  if (x < 8 && x > -18 && y < -12) return radius > 62 ? "Long On" : "Mid On";
  if (x < 0 && angle < -115) return radius > 62 ? "Deep Midwicket" : "Midwicket";
  if (x < 0 && angle >= -180 && angle < -145) return radius > 62 ? "Deep Square Leg" : "Square Leg";
...
const getCoverageAt = (fielders: Fielder[], target: Point, skill: keyof Pick<Player, "catching" | "speed" | "throwing">) => {
  const nearest = Math.min(...fielders.map((fielder) => distance(fielder, target)));
...
const analyzeField = (fielders: Fielder[], context: MatchContext): Analysis => {
  ...
  const closeCatchers = fielders.filter((fielder) => Math.hypot(fielder.x, fielder.y) < 32).length;
  const boundaryRiders = fielders.filter((fielder) => Math.hypot(fielder.x, fielder.y) > 62).length;
  ...
  const phaseAggression =
    context.phase === "Powerplay" ? 12 : context.phase === "Death Overs" ? -8 : context.format === "Test" ? 8 : 0;
  const attackScore = clamp(closeCatchers * 12 + ringFielders * 4 + phaseAggression + (catchingRating - 70), 18, 98);
  const defenseScore = clamp(boundaryRiders * 13 + speedRating - 38 + (context.phase === "Death Overs" ? 15 : 0), 20, 98);
  const boundaryScore = clamp(boundaryRiders * 15 + getCoverageAt(fielders, { x: 0, y: -88 }, "speed") / 4, 12, 98);
  ...
  const balance = clamp(100 - Math.abs(offSide - legSide) * 9 + (ringFielders - 3) * 3, 18, 96);
  const offStrength = clamp(offSide * 13 + getCoverageAt(fielders, { x: 75, y: -20 }, "speed") / 3, 18, 98);
  const legStrength = clamp(legSide * 13 + getCoverageAt(fielders, { x: -75, y: -20 }, "speed") / 3, 18, 98);
  const weakZones = [
    offSide < 4 ? "Cover-point corridor" : "",
    legSide < 4 ? "Square-leg release lane" : "",
    boundaryRiders < 4 && context.phase === "Death Overs" ? "Straight boundary arc" : "",
    closeCatchers < 3 && ["Fast", "Swing", "Seam"].includes(context.bowlerType) ? "Slip cordon pressure" : "",
  ].filter(Boolean);
  const gaps = shots
    .filter((shot) => getCoverageAt(fielders, shot.path[shot.path.length - 1], "speed") < 56)
    .slice(0, 4)
    .map((shot) => shot.zone);
  const hasKeeper = fielders.some((fielder) => detectPosition(fielder, context.battingHand) === "Wicket Keeper");
  const hasBowler = fielders.some((fielder) => detectPosition(fielder, context.battingHand) === "Bowler");

  const misplaced = [
    ...(!hasKeeper ? ["Wicket Keeper is out of position (must be behind striker stumps)"] : []),
    ...(!hasBowler ? ["Bowler is out of position (must be at bowler's end stumps)"] : []),
    ...fielders
      .filter((fielder) => {
        const detected = detectPosition(fielder, context.battingHand);
        if (detected === "Wicket Keeper" || detected === "Bowler") return false;
        return !fielder.preferred.some((position) => detected.includes(position) || position.includes(detected));
      })
      .slice(0, 3)
  ...
  return {
    ...
    weakZones: weakZones.length ? weakZones : ["No critical structural weakness"],
    gaps: gaps.length ? gaps : ["No obvious scoring gap"],
    misplaced: misplaced.length ? misplaced : ["Personnel alignment is strong"],
    risks: [
      boundaryScore < 58 ? "Boundary riders are too narrow for death-over bowling" : "Boundary shell is stable",
      Math.abs(offStrength - legStrength) > 28 ? "Field is leaning heavily to one side" : "Side-to-side split is controlled",
      catchScore < 55 ? "Low wicket-taking pressure around the bat" : "Catching pressure is credible",
    ],
    recommendations: [
      attackScore < 55 ? "Pull one sweeper into catching cover for the next two balls" : "Keep current wicket-taking pressure",
      context.pitch === "Green" ? "Retain gully and third slip until movement fades" : "Use a ring fielder to choke singles",
      context.phase === "Death Overs" ? "Protect long off, long on, deep square and third man" : "Keep one fielder saving one on the off side",
    ],
  };
};
...
const normalizeFielders = (loadedFielders: Fielder[]) =>
  loadedFielders.map((fielder, index) => {
    ...
    return {
      ...
      ...rest,
      name: legacyPlayerNames.has(fielder.name) ? `Player ${index + 1}` : fielder.name || `Player ${index + 1}`,
    };
  });
...
const getInitialFielders = (): Fielder[] => {
  ...
  if (urlFormation) {
    try {
      const decoded = JSON.parse(atob(urlFormation)) as { fielders?: Fielder[] };
  ...
  try {
    ...
    return parsed.fielders?.length === 11 ? normalizeFielders(parsed.fielders) : initialFielders;
  } catch {
...
const getInitialContext = (): MatchContext => {
  ...
  if (urlFormation) {
    try {
      const decoded = JSON.parse(atob(urlFormation)) as { context?: MatchContext };
      if (decoded.context) {
  ...
  try {
    ...
    return parsed.context ? { ...contextDefaults, ...parsed.context } : contextDefaults;
  } catch {
...
function App() {
  ...
  const [stableFielders, setStableFielders] = useState<Fielder[]>(getInitialFielders);
  const [context, setContext] = useState<MatchContext>(getInitialContext);
  const [labelMode, setLabelMode] = useState<"name" | "position" | "both">("both");
  ...
  const [notes, setNotes] = useState("Plan A: squeeze cover-point, keep long off finer, and force the power hitter square.");
  const [voiceNotes, setVoiceNotes] = useState<string[]>([]);
  const [timeline, setTimeline] = useState<string[]>(["New match board created", "Powerplay aggressive base loaded"]);
  ...
  const analysis = useMemo(() => analyzeField(stableFielders, context), [stableFielders, context]);
  const radarData = useMemo(
    () => Object.entries(analysis.scores).map(([metric, value]) => ({ metric, value })),
    [analysis],
  );
  ...
  const comparison = useMemo(() => {
    ...
    return [
      ["Coverage Difference", saved ? analysis.scores["Field Balance"] - saved.scores["Field Balance"] : 0],
      ["Catch Difference", saved ? analysis.scores["Catching Opportunity"] - saved.scores["Catching Opportunity"] : 0],
      ["Weakness Comparison", saved ? saved.weakZones.length - analysis.weakZones.length : 0],
      ["Boundary Risk Comparison", saved ? analysis.scores["Boundary Protection"] - saved.scores["Boundary Protection"] : 0],
    ];
  }, [analysis, context, savedFormation]);
  const activeTutorialStep = tutorialSteps[tutorialStep];
  const focusClass = (target: TutorialTarget) =>
    tutorialOpen && activeTutorialStep.target === target ? " tutorial-focus" : "";

  useEffect(() => {
    localStorage.setItem(storageKey, JSON.stringify({ fielders, context, notes, savedFormation }));
  ...
  const boardPointFromEvent = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    return {
      x: clamp(adjustedX * Math.cos(radians) - adjustedY * Math.sin(radians), -96, 96),
      y: clamp(adjustedX * Math.sin(radians) + adjustedY * Math.cos(radians), -96, 96),
    };
  };

  const updateFielderPosition = (id: number, point: Point) => {
    setFielders((current) =>
      current.map((fielder) => (fielder.id === id ? { ...fielder, x: point.x, y: point.y } : fielder)),
    );
  };

  const updatePlayerName = (id: number, name: string) => {
    const next = fielders.map((fielder) => (fielder.id === id ? { ...fielder, name } : fielder));
    setFielders(next);
    setStableFielders(next);
    setHistory((current) =>
      current.map((snapshot, index) =>
        index === historyIndex
          ? snapshot.map((fielder) => (fielder.id === id ? { ...fielder, name } : fielder))
          : snapshot,
      ),
    );
  };
  ...
  const handlePointerMove = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (panStart && dragStart) {
      ...
      setPan({ x: clamp(panStart.x + x - dragStart.x, -42, 42), y: clamp(panStart.y + y - dragStart.y, -42, 42) });
    }
  };
  ...
  const handlePointerUp = () => {
    if (activeId) {
      setFielders((current) => {
        ...
        setHistory((items) => [...items.slice(0, historyIndex + 1), currentSnapshot].slice(-30));
  ...
  const resetBoard = () => {
    ...
    setRotation(0);
    commitFielders(
      fielders.map((fielder, index) => ({ ...fielder, x: initialFielders[index].x, y: initialFielders[index].y })),
  ...
  const exportPng = async () => {
    ...
    link.download = "cricket-field-strategy.png";
  ...
  const exportPdf = async () => {
    if (!boardRef.current) return;
    const canvas = await html2canvas(boardRef.current, { backgroundColor: "#071014", scale: 2 });
  ...
  const importFormation = (file: File) => {
    ...
    reader.onload = () => {
      try {
        const parsed = JSON.parse(String(reader.result)) as { fielders: Fielder[] };
        if (parsed.fielders?.length === 11) {
          const next = normalizeFielders(parsed.fielders);
          commitFielders(next, "Imported formation loaded");
        }
      } catch {
        setTimeline((items) => ["Import failed: invalid formation file", ...items].slice(0, 8));
      }
    };
  ...
  const runTutorialAction = () => {
    switch (activeTutorialStep.action) {
      case "Use T20 Powerplay":
        setContext((current) => ({ ...current, format: "T20", phase: "Powerplay", battingStyle: "Aggressive" }));
        break;
      ...
      case "Save Setup":
        ...
        setTimeline((items) => ["Tutorial saved a comparison setup", ...items].slice(0, 8));
        break;
    ...
    <main className={theme === "dark" ? "app-shell dark" : "app-shell light"}>
      ...
      >
        ...
        <div className="top-actions">
          <button
            className={tutorialOpen ? "tutorial-button active" : "tutorial-button"}
            onClick={() => {
          ...
          <IconButton label="Redo" onClick={redo} disabled={historyIndex >= history.length - 1}>
            <Redo2 size={18} />
          </IconButton>
          <IconButton label={locked ? "Unlock positions" : "Lock positions"} onClick={() => setLocked((value) => !value)}>
            {locked ? <Lock size={18} /> : <Unlock size={18} />}
          </IconButton>
          <IconButton label="Toggle theme" onClick={() => setTheme((value) => (value === "dark" ? "light" : "dark"))}>
            {theme === "dark" ? <Sun size={18} /> : <Moon size={18} />}
          </IconButton>
          <button className="primary-button" onClick={() => setAnalysisRun((value) => value + 1)}>
            <Bot size={18} />
            Analyze Field
          </button>
        </div>
      ...
      <div className="workspace">
        <aside className="left-rail">
          <Panel title="Match Context" icon={<Activity size={18} />} className={focusClass("context")}>
            <SelectGroup
              label="Batting Hand"
              value={context.battingHand}
              options={["Right Handed", "Left Handed"]}
              onChange={(battingHand) => setContext((current) => ({ ...current, battingHand }))}
            />
            <SelectGroup
              label="Batting Style"
              value={context.battingStyle}
              options={["Aggressive", "Anchor", "Power Hitter", "Finisher"]}
              onChange={(battingStyle) => setContext((current) => ({ ...current, battingStyle }))}
            />
            <SelectGroup
              label="Bowler Type"
              value={context.bowlerType}
              options={["Fast", "Swing", "Seam", "Off Spinner", "Leg Spinner", "Left Arm Orthodox", "Mystery Spinner"]}
              onChange={(bowlerType) => {
                setContext((current) => ({ ...current, bowlerType }));
                const isSpin = ["Off Spinner", "Leg Spinner", "Left Arm Orthodox", "Mystery Spinner"].includes(bowlerType);
                setFielders((currentFielders) => {
                  const nextFielders = currentFielders.map((fielder) => {
                    if (fielder.role === "Keeper" && fielder.x === 0 && (fielder.y === 48 || fielder.y === 35 || fielder.y === 42)) {
            ...
            <SelectGroup
              ...
              onChange={(pitch) => setContext((current) => ({ ...current, pitch }))}
            />
            <div className="two-col">
              <SelectGroup
                ...
                onChange={(format) => setContext((current) => ({ ...current, format }))}
              />
              <SelectGroup
                label="Phase"
                value={context.phase}
                options={["Powerplay", "Middle Overs", "Death Overs"]}
                onChange={(phase) => setContext((current) => ({ ...current, phase }))}
              />
            </div>
          </Panel>

          <Panel title="Formation Presets" icon={<Layers3 size={18} />} className={focusClass("presets")}>
            <div className="preset-section">
              <p>Fast Bowling Attack</p>
              {["Test Match Attack", "Swing Attack", "New Ball Setup"].map((preset) => (
                <PresetButton key={preset} label={preset} onClick={() => applyPreset(preset)} />
              ))}
            </div>
            <div className="preset-section">
            ...
            <div className="preset-section">
              <p>Limited Overs</p>
              {["Powerplay Aggressive", "Powerplay Defensive", "Middle Overs Choke", "Death Overs Protection"].map((preset) => (
                <PresetButton key={preset} label={preset} onClick={() => applyPreset(preset)} />
              ))}
            </div>
          </Panel>
          ...
          <Panel title="Players" icon={<Users size={18} />}>
            <div className="player-list">
              {fielders.map((fielder) => (
                <motion.div className="player-card" key={fielder.id} whileHover={{ y: -3, scale: 1.01 }}>
                  <div className="player-swatch" style={{ borderColor: fielder.color, background: `${fielder.color}22` }} />
                  <div className="player-meta">
                    <input
                      aria-label={`Name for player ${fielder.id}`}
                      value={fielder.name}
                      onChange={(event) => updatePlayerName(fielder.id, event.target.value)}
                      onFocus={(event) => event.target.select()}
                      onPointerDown={(event) => event.stopPropagation()}
                    />
                    <span>
                      {fielder.role} / {detectPosition(fielder, context.battingHand)}
                    </span>
                    <div className="rating-bars">
                      <i style={{ width: `${fielder.skill}%`, background: fielder.color }} />
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        <section className={`field-stage${focusClass("board")}`}>
          <div className="stage-toolbar">
            <div className={`toolbar-group${focusClass("save")}`}>
              <IconButton label="Zoom in" onClick={() => setZoom((value) => clamp(value + 0.12, 0.68, 1.75))}>
                <ZoomIn size={18} />
              </IconButton>
              <IconButton label="Zoom out" onClick={() => setZoom((value) => clamp(value - 0.12, 0.68, 1.75))}>
                <ZoomOut size={18} />
              </IconButton>
              <IconButton label="Reset positions" onClick={resetBoard}>
                <RotateCcw size={18} />
              </IconButton>
              <IconButton label="Fullscreen tactical board" onClick={toggleFullscreen}>
                <Maximize2 size={18} />
              </IconButton>
            </div>
            ...
            <div className="label-selector-group">
              ...
              <div className="label-toggle-buttons">
                {(["position", "name", "both"] as const).map((mode) => (
                  ...
                  >
                    {mode === "position" ? "Pos" : mode === "name" ? "Name" : "Both"}
                  </button>
                ))}
              </div>
            </div>
            <div className="toolbar-group">
              <IconButton label="Save formation" onClick={() => setSavedFormation(fielders.map((fielder) => ({ ...fielder })))}>
                <Save size={18} />
              </IconButton>
          ...
          >
            ...
            <svg
              ...
              onWheel={(event) => {
                event.preventDefault();
                setZoom((value) => clamp(value + (event.deltaY > 0 ? -0.08 : 0.08), 0.68, 1.75));
              }}
            >
              <defs>
                <filter id="glow">
              ...
              <g transform={`translate(${pan.x} ${pan.y}) scale(${zoom}) rotate(${rotation})`}>
                <circle cx="0" cy="0" r="111" fill="#0b1220" opacity="0.68" />
                <circle cx="0" cy="0" r="101" fill="url(#grass)" stroke="#b7ffcc" strokeDasharray="3 6" strokeWidth="1.2" />
                <circle cx="0" cy="0" r="94" fill="none" stroke="#e7ff9d" strokeOpacity="0.55" strokeWidth="1.3" />
                <circle cx="0" cy="0" r="48" fill="none" stroke="#ddffef" strokeDasharray="2 3" strokeOpacity="0.65" strokeWidth="1" />
                <g opacity="0.2">
                  {Array.from({ length: 18 }).map((_, index) => (
                    <line key={index} x1="0" y1="-101" x2="0" y2="101" stroke="#dcfce7" strokeWidth="0.4" transform={`rotate(${index * 10})`} />
                  ))}
                </g>
                <rect x="-7.5" y="-38" width="15" height="76" rx="2" fill="url(#pitch)" stroke="#ffefc7" strokeWidth="0.8" />
                <line x1="-16" y1="-28" x2="16" y2="-28" stroke="#fff7dd" strokeWidth="0.9" />
                <line x1="-16" y1="28" x2="16" y2="28" stroke="#fff7dd" strokeWidth="0.9" />
                <line x1="-11" y1="-34" x2="-11" y2="-20" stroke="#fff7dd" strokeWidth="0.65" />
                <line x1="11" y1="-34" x2="11" y2="-20" stroke="#fff7dd" strokeWidth="0.65" />
                <line x1="-11" y1="20" x2="-11" y2="34" stroke="#fff7dd" strokeWidth="0.65" />
                <line x1="11" y1="20" x2="11" y2="34" stroke="#fff7dd" strokeWidth="0.65" />
                <Wickets y={-31} />
                <Wickets y={31} />
                ...
                <AnimatePresence>
                  {activeShot && (() => {
                    const isSpin = ["Off Spinner", "Leg Spinner", "Left Arm Orthodox", "Mystery Spinner"].includes(context.bowlerType);
                    const bowlTime = isSpin ? 0.75 : 0.45;
                    const shotTime = 1.25;
                    const totalTime = bowlTime + shotTime;
                    const xKeyframes = [0, 0, activeShot.path[1].x, activeShot.path[2].x];
                    const yKeyframes = [-31, 28, activeShot.path[1].y + 28, activeShot.path[2].y + 28];
                    const times = [0, bowlTime / totalTime, (bowlTime + 0.5) / totalTime, 1];
                    const rKeyframes = [2.5, 2.8, activeShot.name.includes("Lofted") || activeShot.name.includes("Hook") || activeShot.name.includes("Upper") ? 6 : 3.5, 2.2];
                    ...
                    return (
                      ...
                      >
                        ...
                        <motion.path
                          ...
                          transition={{ delay: bowlTime, duration: shotTime, ease: "easeOut" }}
                        />
                        ...
                        <motion.circle
                          ...
                          transition={{ delay: bowlTime, duration: 0.35, ease: "easeOut" }}
                        />
                ...
                {fielders.map((fielder) => {
                  ...
                  return (
                    <motion.g
                      ...
                      transition={
                        activeId === fielder.id
                          ...
                          : { type: "spring", stiffness: 220, damping: 24, mass: 0.75 }
                      }
                      onPointerDown={(event) => {
                    ...
                    >
                      <circle r="7.8" fill="#071014" stroke={fielder.color} strokeWidth="2.2" filter="url(#glow)" />
                      <circle r="4.4" fill={fielder.color} opacity="0.9" />
                      <g transform="translate(10 -10)">
                        <rect x="0" y="0" width={labelWidth} height="10" rx="3" fill="#061117" stroke="rgba(255,255,255,0.12)" strokeWidth="0.5" opacity="0.88" />
            ...
            <div className="board-hud">
              ...
              <MetricPill label="Mode" value={locked ? "Locked" : "Live Drag"} />
            </div>
          </motion.div>

          <div className="simulation-row">
            <Panel title="Shot Simulation" icon={<Play size={18} />} className={focusClass("shots")}>
              <div className="shot-grid">
                {shots.map((shot) => (
                  <button
                    className={activeShot?.name === shot.name ? "shot-chip active" : "shot-chip"}
                    key={shot.name}
                    onClick={() => {
                      setActiveShot(shot);
                      setShotRun((value) => value + 1);
            ...
            <Panel title="Heatmap Analytics" icon={<Target size={18} />} className={focusClass("heatmap")}>
              <div className="heatmap-tabs">
                {heatmapModes.map((mode) => (
                  <button className={mode === heatmap ? "active" : ""} key={mode} onClick={() => setHeatmap(mode)}>
                    {mode}
                  </button>
                ))}
              </div>
        ...
        <aside className="right-rail">
          <Panel title="AI Coach" icon={<Bot size={18} />} className={focusClass("coach")}>
            <motion.div key={analysisRun} className="score-orbit" initial={{ scale: 0.92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
              <div>
            ...
            <InsightList title="Misplaced Fielders" items={analysis.misplaced} />
            <InsightList title="High-Risk Areas" items={analysis.risks} />
            <InsightList title="Recommended Changes" items={analysis.recommendations} />
          </Panel>
            ...
            <div className="chart-box">
              <ResponsiveContainer height={210} width="100%">
                <RadarChart data={radarData}>
                  ...
                  <PolarAngleAxis dataKey="metric" tick={{ fill: "rgba(226,232,240,0.7)", fontSize: 9 }} />
                  <RadarShape dataKey="value" stroke="#22d3ee" fill="#22d3ee" fillOpacity={0.34} />
                  <Tooltip contentStyle={{ background: "#061117", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8 }} />
                </RadarChart>
              </ResponsiveContainer>
            </div>
            <div className="chart-box">
              <ResponsiveContainer height={170} width="100%">
                <BarChart data={barData}>
                  <XAxis dataKey="metric" stroke="rgba(226,232,240,0.55)" fontSize={10} />
              ...
              {comparison.map(([label, value]) => (
                <div key={label}>
                  ...
                  <strong className={Number(value) >= 0 ? "positive" : "negative"}>
                    {savedFormation ? `${Number(value) > 0 ? "+" : ""}${value}` : "Save setup"}
                  </strong>
                </div>
              ))}
            </div>
      ...
      <motion.section className={`${coachOpen ? "coach-drawer open" : "coach-drawer"}${focusClass("coachMode")}`} layout>
        <button className="coach-toggle" onClick={() => setCoachOpen((value) => !value)}>
          ...
          {coachOpen && (
            ...
            >
              ...
              <div className="coach-actions">
                ...
                <button onClick={() => setTimeline((items) => ["Version checkpoint saved", ...items].slice(0, 8))}>
                  <History size={17} />
                  Version
                </button>
              </div>
              <div className="timeline-grid">
                ...
                <div>
                  <h3>Voice Notes</h3>
                  {(voiceNotes.length ? voiceNotes : ["No voice notes captured"]).map((item) => (
                    <p key={item}>{item}</p>
                  ))}
                </div>
                <div>
                  <h3>Opposition Analysis</h3>
                  <p>{context.battingStyle} batter against {context.bowlerType.toLowerCase()} on a {context.pitch.toLowerCase()} pitch.</p>
        ...
        {tutorialOpen && (
          ...
          >
            <div className="tutorial-agent-header">
              ...
              <IconButton label="Close tutorial" onClick={() => setTutorialOpen(false)}>
                <X size={17} />
              </IconButton>
            </div>
            <div className="tutorial-progress">
              <i style={{ width: `${((tutorialStep + 1) / tutorialSteps.length) * 100}%` }} />
            </div>
            ...
            <div className="tutorial-actions">
              ...
              <button disabled={tutorialStep === 0} onClick={() => setTutorialStep((step) => Math.max(0, step - 1))}>
                Back
              </button>
              <button
                onClick={() => {
    ...
    <div className="animated-backdrop" aria-hidden="true">
      {Array.from({ length: 28 }).map((_, index) => (
        <motion.span
          ...
          animate={{ opacity: [0, 0.75, 0], y: [-20, -130], x: [0, index % 2 ? 26 : -22] }}
          transition={{ duration: 5 + (index % 6), repeat: Infinity, delay: index * 0.28 }}
          style={{ left: `${(index * 37) % 100}%`, top: `${28 + ((index * 19) % 68)}%` }}
        />
      ))}
    </div>
  );
...
function Wickets({ y }: { y: number }) {
  return (
    <g transform={`translate(0 ${y})`}>
      {[-2.4, 0, 2.4].map((x) => (
        <line key={x} x1={x} y1="-4" x2={x} y2="4" stroke="#f8fafc" strokeLinecap="round" strokeWidth="0.85" />
      ))}
    </g>
  );
}
...
function Heatmap({ mode, fielders }: { mode: string; fielders: Fielder[] }) {
  const zones = [
    { x: 74, y: -48, value: mode === "Boundary Risk" ? 76 : getCoverageAt(fielders, { x: 74, y: -48 }, "speed"), color: "#ef4444" },
    { x: -70, y: -38, value: mode === "Scoring Zones" ? 82 : getCoverageAt(fielders, { x: -70, y: -38 }, "speed"), color: "#f97316" },
    { x: 58, y: 42, value: mode === "Catch Probability" ? getCoverageAt(fielders, { x: 22, y: 12 }, "catching") : 52, color: "#22d3ee" },
    { x: -28, y: 66, value: mode === "Safe Singles" ? 80 : getCoverageAt(fielders, { x: -28, y: 66 }, "throwing"), color: "#a3e635" },
    { x: 0, y: -82, value: mode === "Two-Run Opportunities" ? 74 : getCoverageAt(fielders, { x: 0, y: -82 }, "speed"), color: "#e879f9" },
    { x: 28, y: -18, value: mode === "Field Coverage" ? 90 : getCoverageAt(fielders, { x: 28, y: -18 }, "speed"), color: "#14b8a6" },
  ];
  return (
    <motion.g key={mode} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      {zones.map((zone) => (
        <motion.circle
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: "10px", opacity: 0.6, transition: "transform 0.2s", transform: isOpen ? "rotate(180deg)" : "rotate(0deg)" }}>
          ▼
        </span>
      </div>
      <AnimatePresence initial={false}>
...
function IconButton({
  ...
  return (
    <button aria-label={label} className="icon-button" disabled={disabled} title={label} onClick={onClick}>
      {children}
    </button>
  );
}
...
function PresetButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <motion.button className="preset-button" onClick={onClick} whileHover={{ x: 4 }} whileTap={{ scale: 0.98 }}>
      <Wand2 size={15} />
      {label}
    </motion.button>
  );
